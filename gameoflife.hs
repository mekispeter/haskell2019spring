...
Call main and choose options, eg:
> main
Animation (y/n):
n
Number of updates:
...
-- updates a grid g n times
-- if anim == True: printing each step,
--                  waiting for Enter between the steps
game :: Cellgrid -> Int -> Bool -> IO()
game g n anim
  | n == 0        = putStrLn(show_grid g)
  | anim == True  = do
    replicateM_ 50   (putStrLn (""))
    putStrLn (show_grid g)
    _ <- getLine
    game (update g) (n-1) anim
  | otherwise     = game (update g) (n-1) anim

main :: IO()
main = do
  putStrLn "Animation (y/n):"
  ch <- getLine
  putStrLn "Number of generations:"
  sn <-getLine
  putStrLn "Pattern (0-4):"
  sm <-getLine
  game (read_grid (samples !! (read sm :: Int))) (read sn :: Int) (if ch == "y" then True else False)
