...
truthValue :: Formula -> [(Index,Bool)] -> Bool
truthValue (At i) evaluation  = unJust (lookup i evaluation) where
    unJust :: Maybe a -> a
    unJust Nothing    = error "Wrong index!"
    unJust (Just x)   = x
truthValue (Neg f)   evaluation = not $ truthValue f evaluation
truthValue (Con f g) evaluation = truthValue f evaluation &&
                                  truthValue g evaluation
truthValue (Dis f g) evaluation = truthValue f evaluation ||
                                  truthValue g evaluation
truthValue (Imp f g) evaluation = not (truthValue f evaluation) ||
                                  truthValue g evaluation
truthValue (Bic f g) evaluation = truthValue f evaluation ==
                                  truthValue g evaluation

truthConditions :: Formula -> [Index] -> [Bool]
truthConditions f atomList = map (truthValue f) (truthPoss atomList)

tautology :: Formula -> Bool
tautology f = all (== True) $ truthConditions f (atoms f)

allAtoms :: [Formula] -> [Index]
allAtoms formulas = foldr reducePlus [] (map atoms formulas)

equivalent :: Formula -> Formula -> Bool
equivalent f g =  truthConditions f (allAtoms [f, g]) ==
                  truthConditions g (allAtoms [f, g])

sample3 = (At 0 `Imp` (At 1 `Imp` At 2))
          `Imp`
          ((At 0 `Imp` At 1) `Imp` (At 0 `Imp` At 2))
